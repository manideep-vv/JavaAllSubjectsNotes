    ...
    private static void send100Messages(Properties properties) {
        ...
        Callback c=((metadata, exception) -> {
           if(exception!=null){
               ...
               System.out.printf("\n sent successfully got ack and msg was written to partition %s with offset %s",metadata.partition(),metadata.offset());
           }
        });
        IntStream.rangeClosed(0,10).forEach(ele->{
            IntStream.range(0,10).forEach(e->{
                System.out.println("key--> "+e);
                ProducerRecord<Integer,String> rec=new ProducerRecord<>("wellsfargoo",e,"ori ne ayya"+e);
                producer.send(rec,c);
            });
            try {
                Thread.sleep(500);
